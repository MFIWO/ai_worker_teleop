...
def generate_launch_description():
    ...
    vr_controller_node = Node(
        ...
        # VR elbow targets must use the same robot arm geometry as wrist goals.
        # Preserve the existing direct elbow input for the physical leader.
        remappings=[
            ('/r_elbow_pose', PythonExpression([
                "'/r_subgoal_pose' if '", controller_type,
                "' == 'vr' else '/r_elbow_pose'",
            ])),
            ('/l_elbow_pose', PythonExpression([
                "'/l_subgoal_pose' if '", controller_type,
                "' == 'vr' else '/l_elbow_pose'",
            ])),
        ],
        parameters=[
            config_file,
    ...
    arm_retargeting_node = Node(
        ...
        parameters=[{'urdf_path': follower_urdf_path, 'base_frame': base_frame}],
        output='screen',
        condition=IfCondition(
            PythonExpression(["'", controller_type, "' == 'vr'"])
        ),
